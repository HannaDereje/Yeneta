...
module.exports = (server) => {
    ...
    server.post("/forwardToEmail", 
        body('email').isEmail().notEmpty(),
        new UserController(userServ, roleServ).ResetPassword)
        
    server.put("/reset/:accessToken", 
        body('password').isLength({min:8}), 
        new UserController(userServ, roleServ).verifyPassword)

    server.get("/getCurrentUser", 
        new UserController(userServ, roleServ).verifyToken,
        new UserController(userServ, roleServ).getCurrentUser)
